...
\usepackage{tikz} % if this line occurs behind the next, an error message should appear. 
\usepackage{fontspec,ctable,colortbl,kantlipsum}
\usepackage{caption}
\usepackage{hyperref}
...
\setupctable{captionskip=-1.68ex}
\begin{document}
\maketitle
\listoftables
...
\chapter{Caption with footnote}
\ctable[
  caption = xCtable without a tmark,
    label = notmark,
      pos = ht
]{ll}{}{\FL
  Hello World!& How are you today? \LL
}

\ctable[
  caption = {Ctable with a tmark\tmark},
    label = tmark,
      pos = ht
]{ll}{\tnote{note}}{\FL
  Hello World!& How are you today? \LL
...
\noindent
Note: tmark may not have an argument in the caption!\\
The table titled \nameref{notmark}\\
The table titled \nameref{tmark}\\
